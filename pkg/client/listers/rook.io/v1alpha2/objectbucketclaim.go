...
type ObjectBucketClaimLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.ObjectBucketClaim, err error)
...
// List lists all ObjectBucketClaims in the indexer.
func (s *objectBucketClaimLister) List(selector labels.Selector) (ret []*v1alpha2.ObjectBucketClaim, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.ObjectBucketClaim))
	})
	return ret, err
}
...
type ObjectBucketClaimNamespaceLister interface {
	...
	Get(name string) (*v1alpha2.ObjectBucketClaim, error)
...
// List lists all ObjectBucketClaims in the indexer for a given namespace.
func (s objectBucketClaimNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.ObjectBucketClaim, err error) {
...
// Get retrieves the ObjectBucketClaim from the indexer for a given namespace and name.
func (s objectBucketClaimNamespaceLister) Get(name string) (*v1alpha2.ObjectBucketClaim, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("objectbucketclaim"), name)
	}
	return obj.(*v1alpha2.ObjectBucketClaim), nil
}
